...
class JavaScriptSyntaxTests(unittest.TestCase):
    def test_shared_bundle_references_use_real_filename(self):
        ...
        self.assertIn('const runHipotecaBdtExportAction = async (button, busyText, task, errorPrefix) => {', app_js)
        ...
        self.assertIn('await runHipotecaBdtExportAction(hipotecaBdtPrintListado, "Imprimiendo...", openHipotecaBdtListadoPrint, "No se pudo imprimir el listado.");', app_js)
        self.assertIn('() => downloadHipotecaBdtPdf("fichas")', app_js)
        self.assertIn('const service = normalizeBudgetServiceKey(workspaceBudgetForm.querySelector(\'[name="servicio"]\')?.value || "gestoria");', app_js)
        self.assertIn('normalizeBudgetServiceKey(row.servicio || "") === "fincas"', app_js)
        self.assertIn('if (raw.includes("comunidad")) return "fincas";', app_js)
        self.assertIn('if (normalized.includes("comunidad")) return "fincas";', app_js)
        self.assertIn('if (key === "comunidades" || key === "comunidad" || key.includes("comunidad")) key = "fincas";', app_js)
        self.assertIn('if (key.includes("comunidad")) set.add("fincas");', app_js)
        self.assertIn('const normalizedService = normalizeBudgetServiceKey(servicio || "");', app_js)
        self.assertIn('const key = normalizeBudgetServiceKey(service || "");', app_js)
        self.assertIn('const targetName = SERVICE_COMPANY_MAP[getServiceLabelFromNormalized(serviceKey) || service];', app_js)
